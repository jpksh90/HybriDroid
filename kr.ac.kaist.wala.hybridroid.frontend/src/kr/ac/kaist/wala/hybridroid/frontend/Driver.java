...
/**
 * Driver is a basic step for type checking. This class finds all JavaScript files loaded from an
 * Android hybrid application and maps bridges to the JavaScript files.
 *
 * @author Sungho Lee
 */
public class Driver {
  ...
  private <T> void putWebViewMap(Map<InstanceKey, Set<T>> m, InstanceKey k, T t) {
    if (!m.containsKey(k)) m.put(k, new HashSet<T>());
    m.get(k).add(t);
  }
   ...
   */
  private Map<InstanceKey, Set<File>> getWebViewJavaScriptMapping(
      PointerAnalysis<InstanceKey> pa, AndroidStringAnalysis asa, String dirPath)
    ...
    for (File html : htmls) {
      File f =
          WebUtil.extractScriptFromHTML(html.toURI().toURL(), DefaultSourceExtractor.factory).snd;
        ...
        for (String v : vs) {
          if (v.startsWith("javascript:")) { // if it is javascript code, then
            ...
            try {
              ...
              FilePrinter.print(
                  jsFile, new FileOutputStream(dirPath + File.separator + jsFile.getName()));
  ...
  /**
   * Make a map representing the relation between WebView and bridge information attached to the
   * WebView.
   *
   * @param pa a pointer analysis result.
   ...
   */
  private Map<InstanceKey, Set<BridgeInfo>> getWebViewBridgeMapping(
      PointerAnalysis<InstanceKey> pa, CallGraph cg, String dir) {
    ...
    while (in.hasNext()) {
      ...
      while (icsr.hasNext()) {
        ...
        if (!cg.getPossibleTargets(n, csr).contains(addJsIM)) continue;
          ...
          for (SSAAbstractInvokeInstruction invokeInst : ir.getCalls(csr)) {
            ...
            for (InstanceKey ik : pa.getPointsToSet(wvPK)) {
              for (IClass c : bridgeClassSet)
                putWebViewMap(
                    m, ik, new BridgeInfo(bridgeName, new ClassInfo(c, isAboveJELLYBEAN)));
            }
          }
        }
      }
  ...
  private boolean isEqualOrAboveJELLY_BEAN_MR1(YMLParser.YMLData data) {
    ...
    if (name.equals("targetSdkVersion")) {
      ...
      boolean is = true;
      for (YMLParser.YMLData child : data.getChildren()) is &= isEqualOrAboveJELLY_BEAN_MR1(child);
      return is;
    }
  }
   ...
   * @return A map from a JavaScript file to a set of bridge informations which could be used in the
   *     JavaScript file.
   */
  public Map<File, Set<BridgeInfo>> analyzeBridgeMapping(String apkPath, String property) {
    ...
    AndroidStringAnalysis asa =
        analyzeString(LocalFileReader.androidJar(p).getPath(), apkPath, ara);
    ...
    for (InstanceKey wv : wvToJs.keySet()) {
      ...
      for (File f : fs) {
        if (bs != null && !bs.isEmpty()) {
          if (!m.containsKey(f)) m.put(f, new HashSet<BridgeInfo>());
          m.get(f).addAll(bs);
        }
      }
    }
