...
/**
 * Check which API-misuses can occur at call sites at which JS call statement invokes Java bride
 * method. This is implemented as singleton object, because type warnings just would be collected
...
public class HybridAPIMisusesChecker {
  ...
  public static HybridAPIMisusesChecker getInstance() {
    if (instance == null) instance = new HybridAPIMisusesChecker();
    return instance;
  }
  ...
  /**
   * API-misuses detection for Java bridge method type overloading. this is implementation for
   * 'Method overloading using parameter type' of the document.
   *
  ...
  public boolean typeOverloadingCheck(IClass objClass, Collection<IMethod> methods) {
    ...
    Map<Pair<String, Integer>, Set<IMethod>> methodMap =
        new HashMap<Pair<String, Integer>, Set<IMethod>>();
    for (IMethod m : possibleMethods) {
      String mName = m.getName().toString();
      int pNum = m.getNumberOfParameters();
      Pair<String, Integer> p = Pair.make(mName, pNum);

      if (methodMap.containsKey(p) == false) methodMap.put(p, new HashSet<IMethod>());
  ...
  /**
   * API-misuses detection for function calls about return assignment. 1) void return assignment
   * check. 2) array return type check. 3) Java to JS type conversion check.
   *
  ...
  private boolean arrayReturnCheck(TypeReference typeRef) {
    if (typeRef.isArrayType()) return false;
    return true;
  }
   ...
   */
  private boolean voidReturnAssignmentCheck(
      CGNode caller, SSAAbstractInvokeInstruction call, TypeReference typeRef) {
  ...
  /**
   * check an argument type which can be comvertable to Java type or not. this is implementation for
   * 'Method invocation with inconvertable type' of the document.
   *
  ...
  public boolean argTypeCheck(
    ...
    if (!AndroidJavaJavaScriptTypeMap.isJs2JavaTypeCompatibleNoImplicitConversion(
        argType, paramType)) {
  ...
  /**
   * check which the number of arguments are same with the number of parameters of the target
   * method.
   *
   * @param caller caller JS method.
   * @param call call JS statement.
   ...
   */
  public boolean argNumCheck(
      CGNode caller, SSAAbstractInvokeInstruction call, IMethod target, int paramNum, int argNum) {
    if (paramNum != argNum) {
  ...
  class ArgsMismatchedWarning implements TypeWarning {
    ...
    public boolean equals(Object o) {
      if (o instanceof ArgsMismatchedWarning) {
        ...
        if (amw.getNode().equals(this.getNode())
            ...
            && amw.getArgNum() == this.getArgNum()) return true;
      }
      return false;
    }
    ...
    public String toString() {
      ...
      msg += "\n\t target: " + getTarget();
      msg +=
          "\n\t the # of parameters is '"
              + getParamNum()
              + "' but the # of arguments is '"
              + getArgNum()
              + "'";
      return msg;
    }
  ...
  class ArrayReturnWarning implements TypeWarning {
    ...
    public ArrayReturnWarning(
        CGNode node, SSAAbstractInvokeInstruction call, IMethod target, TypeReference retType) {
      this.node = node;
      this.call = call;
      this.target = target;
  ...
  class ReturnTypeWarning implements TypeWarning {
    ...
    public int hashCode() {
      return this.getNode().hashCode()
          + this.getCallInst().hashCode()
          + this.getTarget().hashCode();
    }
    ...
    public boolean equals(Object o) {
      if (o instanceof ReturnTypeWarning) {
        ...
        if (rvtw.getCallInst().iIndex() == this.getCallInst().iIndex()
            ...
            && rvtw.toString().equals(this.toString())) return true;
      }
      return false;
    }
  }
  ...
  class ReturnVoidTypeWarning extends ReturnTypeWarning {
    ...
    public boolean equals(Object o) {
      if (o instanceof ReturnVoidTypeWarning) {
        ...
        if (rvtw.getCallInst().iIndex() == this.getCallInst().iIndex()
            && rvtw.getNode().equals(this.getNode())
            && rvtw.getTarget().equals(this.getTarget())) return true;
      }
      return false;
    }
  }
  ...
  class ImplicitParamTypeWarning extends ParamTypeWarning {
    ...
    public ImplicitParamTypeWarning(
        CGNode node,
    ...
    public boolean equals(Object o) {
      if (o instanceof ParamTypeWarning) {
        ...
        if (ptw.getArgNum() == this.getArgNum()
            ...
            && ptw.getArgType().equals(this.getArgType())) return true;
      }
      return false;
    }
  }
  ...
  class MethodTypeOverloadingWarning implements Warning {
    ...
    public MethodTypeOverloadingWarning(
        IClass tclass, String name, int paramNum, Set<IMethod> mSet) {
      this.tclass = tclass;
      this.name = name;
      this.paramNum = paramNum;
      this.mSet = mSet;
