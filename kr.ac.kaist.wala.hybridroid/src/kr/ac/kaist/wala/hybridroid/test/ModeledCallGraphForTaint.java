...
public class ModeledCallGraphForTaint implements CallGraph {
  ...
  public ModeledCallGraphForTaint(CallGraph delegate) {
    ...
    TypeReference activityTR =
        TypeReference.find(ClassLoaderReference.Primordial, "Landroid/app/Activity");
    TypeReference wvTR =
        TypeReference.find(ClassLoaderReference.Primordial, "Landroid/webkit/WebView");
    TypeReference contextTR =
        TypeReference.find(ClassLoaderReference.Primordial, "Landroid/content/Context");
    TypeReference handlerTR =
        TypeReference.find(ClassLoaderReference.Primordial, "Landroid/os/Handler");
  ...
  private SSANewInstruction findNew(IR ir, int def) {
    for (Iterator<NewSiteReference> inew = ir.iterateNewSites(); inew.hasNext(); ) {
      ...
      if (newInst.getDef() == def) return newInst;
    }
    return null;
  }
  ...
  private boolean init_handler() {
    ...
    TypeReference runnableTR =
        TypeReference.find(ClassLoaderReference.Application, "Ljava/lang/Runnable");
    ...
    for (CGNode n : this) {
      ...
      if (ir != null) {
        for (Iterator<CallSiteReference> icsr = ir.iterateCallSites(); icsr.hasNext(); ) {
          ...
          if (targetClass != null
              && (targetClass.equals(handlerClass)
                  && targetMR.getSelector().equals(hs)
                  && targetMR.getNumberOfParameters() > 0)) {
  ...
  private boolean isApplicationCall(CallSiteReference csr) {
    ...
    if (targetClass != null
      ...
      return true;
        ...
        && (targetClass.equals(handlerClass)
            || cha.isSubclassOf(targetClass, handlerClass))) // except for handler
    return true;
    return false;
  }
  ...
  private boolean isRecursion(CGNode n, CallSiteReference csr) {
    ...
    if (targetClass != null)
      if (targetClass.equals(klass) && targetMethodReference.equals(mr)) return true;
    return false;
  }
            ...
            if (isApplicationCall(csr)) {
              if (isRecursion(n, csr)) {
                ...
                updatePossibleSiteMap(Pair.make(n, n), csr);
                  ...
                  csr)) { // we do not expand Android libraries used for backward compatibility.
  ...
  private void modelOfStartActivityToOnCreate() {
    ...
    Set<Pair<CGNode, CallSiteReference>> startActivityNodesAndSites =
        findStartActivityNodesAndSites();
    Set<Pair<CGNode, CallSiteReference>> startActivityForResultNodesAndSites =
        findStartActivityForResultNodesAndSites();
  ...
  private Set<CGNode> findOnCreateNodes() {
    if (findOnCreateNodesCache != null) return findOnCreateNodesCache;

    Set<CGNode> res = new HashSet<CGNode>();
    Selector oncreateSelector = Selector.make("onCreate(Landroid/os/Bundle;)V");

    for (CGNode n : this) {
      if (isAndroidLibrary(n)) continue;
      IClass klass = n.getMethod().getDeclaringClass();
      Selector selector = n.getMethod().getSelector();
      if (klass != null
          && (klass.equals(activityClass) || cha.isSubclassOf(klass, activityClass))
          && selector.equals(oncreateSelector)
          && ((MODEL_APPLICATION_ONLY) ? isJavaNode(n) : true)) res.add(n);
    }
  ...
  private Set<Pair<CGNode, CallSiteReference>> findStartActivityNodesAndSites() {
    if (findStartActivityNodesAndSitesCache != null) return findStartActivityNodesAndSitesCache;
      ...
      if (klass != null && ((MODEL_APPLICATION_ONLY) ? isJavaNode(n) : true)) {
        if (n.getIR() != null)
          for (Iterator<CallSiteReference> icsr = n.getIR().iterateCallSites(); icsr.hasNext(); ) {
            ...
            if (targetClass != null
                && (targetClass.equals(contextClass) || cha.isSubclassOf(targetClass, contextClass))
                && targetMR.getSelector().equals(startActivitySelector)) {
  ...
  private Set<Pair<CGNode, CallSiteReference>> findStartActivityForResultNodesAndSites() {
    ...
    Set<Pair<CGNode, CallSiteReference>> res = new HashSet<Pair<CGNode, CallSiteReference>>();
    Selector startActivityForResultSelector =
      ...
      if (klass != null && ((MODEL_APPLICATION_ONLY) ? isJavaNode(n) : true)) {
        if (n.getIR() != null)
          for (Iterator<CallSiteReference> icsr = n.getIR().iterateCallSites(); icsr.hasNext(); ) {
            ...
            if (targetClass != null
                && (targetClass.equals(contextClass) || cha.isSubclassOf(targetClass, contextClass))
                && targetMR.getSelector().equals(startActivityForResultSelector)) {
              res.add(Pair.make(n, csr));
            }
          }
      }
  ...
  private Set<CGNode> findOnActivityResultNodes() {
    if (findOnActivityResultNodesCache != null) return findOnActivityResultNodesCache;

    Set<CGNode> res = new HashSet<CGNode>();
    Selector oncreateSelector = Selector.make("onActivityResult(IILandroid/content/Intent;)V");

    for (CGNode n : this) {
      if (isAndroidLibrary(n) && ((MODEL_APPLICATION_ONLY) ? !isJavaNode(n) : !true)) continue;
  ...
  private void modelOfRequestPermissionToOnRequestPermissionsResult() {
    Set<Pair<CGNode, CallSiteReference>> requestPermissionNodesAndSites =
        findRequestPermissionNodesAndSites();
  ...
  private Set<Pair<CGNode, CallSiteReference>> findLoadUrlNodesAndSites() {
    if (findLoadUrlNodesAndSitesCache != null) return findLoadUrlNodesAndSitesCache;
    ...
    for (CGNode n : this) {
      if (n.getIR() != null)
        for (Iterator<CallSiteReference> icsr = n.getIR().iterateCallSites(); icsr.hasNext(); ) {
          ...
          if (targetClass != null
              && (targetClass.equals(webViewClass) || cha.isSubclassOf(targetClass, webViewClass))
              && targetMR.getSelector().equals(startActivityForResultSelector)
              && ((MODEL_APPLICATION_ONLY) ? isJavaNode(n) : true)) {
            res.add(Pair.make(n, csr));
          }
        }
    }
  ...
  private Set<CGNode> findAllJavaScriptNodes() {
    if (findAllJavaScriptNodesCache != null) return findAllJavaScriptNodesCache;
    ...
    for (CGNode n : this) {
      if (isJSNode(n)
          && !n.getMethod().isInit()
          && !n.getMethod().isClinit()
          && !n.toString().contains("<ctor for")) res.add(n);
    }
  ...
  public Iterator<CGNode> iterator() {
    ...
    return new Iterator<CGNode>() {
      ...
      public CGNode next() {
        // TODO Auto-generated method stub
        if (iterd.hasNext()) return iterd.next();
        else return iterm.next();
      }
    };
  ...
  public Iterator<CGNode> getPredNodes(CGNode n) {
    ...
    if (predMap.containsKey(n)) res.addAll(predMap.get(n));
  ...
  public int getPredNodeCount(CGNode n) {
    ...
    if (predMap.containsKey(n)) num += predMap.get(n).size();
  ...
  public Iterator<CGNode> getSuccNodes(CGNode n) {
    ...
    if (succMap.containsKey(n)) res.addAll(succMap.get(n));
  ...
  public int getSuccNodeCount(CGNode n) {
    ...
    if (succMap.containsKey(n)) num += succMap.get(n).size();
  ...
  public boolean hasEdge(CGNode src, CGNode dst) {
    ...
    return delegate.hasEdge(src, dst)
        || ((predMap.containsKey(dst)) ? predMap.get(dst).contains(src) : false);
  }

  @Override
  public int getNumber(CGNode N) {
    // TODO Auto-generated method stub
    if (N instanceof UnreachableCGNode) return ((UnreachableCGNode) N).number;
  ...
  public CGNode getNode(int number) {
    // TODO Auto-generated method stub
    if (mNodes.containsKey(number)) return mNodes.get(number);
  ...
  public Iterator<CGNode> iterateNodes(IntSet s) {
    ...
    for (IntIterator ii = s.intIterator(); ii.hasNext(); ) {
      int i = ii.next();
      if (mNodes.containsKey(i)) nodes.add(mNodes.get(i));
      else {
  ...
  public Set<CGNode> getPossibleTargets(CGNode node, CallSiteReference site) {
    // TODO Auto-generated method stub
    Set<CGNode> res = new HashSet<CGNode>();
    if (delegate.containsNode(node)) res.addAll(delegate.getPossibleTargets(node, site));

    Pair<CGNode, CallSiteReference> p = Pair.make(node, site);
    if (possibleTargetMap.containsKey(p)) res.addAll(possibleTargetMap.get(p));
  ...
  public int getNumberOfTargets(CGNode node, CallSiteReference site) {
    ...
    if (delegate.containsNode(node)) num += delegate.getNumberOfTargets(node, site);

    Pair<CGNode, CallSiteReference> p = Pair.make(node, site);
    if (possibleTargetMap.containsKey(p)) num += possibleTargetMap.get(p).size();
  ...
  public Iterator<CallSiteReference> getPossibleSites(CGNode src, CGNode target) {
    ...
    if (delegate.containsNode(src) && delegate.containsNode(target))
      for (Iterator<CallSiteReference> icsr = delegate.getPossibleSites(src, target);
          icsr.hasNext(); ) {
        res.add(icsr.next());
      }
    ...
    if (possibleSiteMap.containsKey(p)) res.addAll(possibleSiteMap.get(p));
