...
public class TaintAnalysisForHybrid {
  private static Atom[][] source = {
    {
      Atom.findOrCreateAsciiAtom("LocationManager"),
  ...
  // pending assign for return value at call black is needed, because there is no return block in
  // supergraph.
  private CallGraph cg;
  private boolean isTainted = false;
  private Map<BasicBlockInContext<IExplodedBasicBlock>, Set<Pair<CGNode, Integer>>>
      pendingAssignForReturn;

  public TaintAnalysisForHybrid(CallGraph cg) {
    this.cg = cg;
    pendingAssignForReturn =
        new HashMap<BasicBlockInContext<IExplodedBasicBlock>, Set<Pair<CGNode, Integer>>>();
    System.out.println("#building super graph...");
    this.supergraph = ICFGSupergraph.make(cg);
    System.out.println("\tdone.");
  }

  private final boolean isSourceTarget(CGNode caller, SSAAbstractInvokeInstruction inst) {
    ...
    for (int i = 0; i < source.length; i++) {
      if (className.equals(source[i][0]) && methodName.equals(source[i][1])) return true;
    }
    return false;
  }

  private final boolean isSinkTarget(CGNode target) {
    Atom className = target.getMethod().getDeclaringClass().getName().getClassName();
    Atom methodName = target.getMethod().getName();
    for (int i = 0; i < sink.length; i++) {
      if (className.equals(sink[i][0]) && methodName.equals(sink[i][1])) return true;
  ...
  private final void pendingReturnAssign(
      BasicBlockInContext<IExplodedBasicBlock> exitBlock, Pair<CGNode, Integer> pair) {
  ...
  private final List<Integer> execReturnAssign(
      BasicBlockInContext<IExplodedBasicBlock> exitBlock,
      TabulationDomain<Pair<CGNode, Integer>, BasicBlockInContext<IExplodedBasicBlock>> domain) {
    List<Integer> dvs = new ArrayList<Integer>(0);
    if (!pendingAssignForReturn.containsKey(exitBlock)) return dvs;
   ...
   */
  private final int calcPropagatedVariable(SSAInstruction inst, int var) {
    if (!inst.hasDef()) return -1;
    ...
    if (inst instanceof JavaScriptCheckReference) {
      ...
      if (propertyReadInst.getUse(0) == var) return propertyReadInst.getDef();
    } else if (inst instanceof JavaScriptPropertyWrite) {
      AstPropertyWrite propertyWriteInst = (AstPropertyWrite) inst;
      if (propertyWriteInst.getUse(2) == var) return propertyWriteInst.getUse(0);
      ...
      if (assignInst.getUse(0) == var) return assignInst.getDef();
      ...
      if (eachGetInst.getUse(0) == var) return eachGetInst.getDef();
      ...
      if (arrayLoadInst.getArrayRef() == var) return arrayLoadInst.getDef();
    } else if (inst instanceof SSAArrayStoreInstruction) {
      SSAArrayStoreInstruction arrayStoreInst = (SSAArrayStoreInstruction) inst;
      if (arrayStoreInst.getUse(0) == var) return arrayStoreInst.getArrayRef();
      ...
      for (int i = 0; i < phiInst.getNumberOfUses(); i++) {
        ...
        if (usedVar == var) return phiInst.getDef();
      }
      ...
      if (putInst.getUse(1) == var) return putInst.getUse(0);
      ...
      if (unaryOpInst.getUse(0) == var) return unaryOpInst.getDef();
      ...
      if (protoLookupInst.getUse(0) == var) return protoLookupInst.getDef();
  ...
  private final BasicBlockInContext<IExplodedBasicBlock> findLastCallBlock(
  ...
  private final List<BasicBlockInContext<IExplodedBasicBlock>> findReturnBlocks(
      BasicBlockInContext<IExplodedBasicBlock> exit) {
    List<BasicBlockInContext<IExplodedBasicBlock>> returnBlocks =
        new ArrayList<BasicBlockInContext<IExplodedBasicBlock>>();
  ...
  public TabulationResult<BasicBlockInContext<IExplodedBasicBlock>, CGNode, Pair<CGNode, Integer>>
    ...
    //	    for(BasicBlockInContext<IExplodedBasicBlock> reachedBB :
    // result.getSupergraphNodesReached()){
  ...
  /** Lee controls numbering of variables for use in tabulation */
  private class TaintDomain extends MutableMapping<Pair<CGNode, Integer>>
  ...
  private class TaintFlowFunctions
        ...
        return new IUnaryFlowFunction() {
          ...
          public IntSet getTargets(int d1) {
            ...
            if (node.equals(srcNode)) {
              ...
              //							for(BasicBlockInContext<IExplodedBasicBlock> exitBB :
              // supergraph.getExitsForProcedure(target)){
                  ...
                  if (var == useVar) {
                    ...
                    if (invokeInst.hasDef()
                        && invokeInst
                            ...
                            .contains("valueOf")) {
                      System.out.println(
                          "\t#Q: " + invokeInst.getDeclaredTarget().getName().toString());
                ...
                if (isSinkTarget(target)) {
                  for (int i = 0; i < invokeInst.getNumberOfUses(); i++) {
                    ...
                    if (var == useVar) {
                      Debug.setDebuggable(this, true);
                      Debug.printMsg(
                          " private data is leaked : the param(" + useVar + ") is tainted.");
                      isTainted = true;
                      Debug.temporaryStop();
                    }
                  }
                    ...
                    if (var == useVar) {
                      ...
                      if (srcNode
                        ...
                        if (invokeInst.isStatic()) {
                          ...
                          //												System.out.println("#2CallInst: "+invokeInst + "\tset:
                          // "+(i-1));
                        } else {
                          d2 = domain.add(Pair.make(target, i));
                          //												System.out.println("#3CallInst: "+invokeInst + "\tset:
                          // "+(i));
                        }
                      else {
                        d2 = domain.add(Pair.make(target, i + 1));
                        //											System.out.println("#4CallInst: "+invokeInst + "\tset: " +
                        // (i+1));
                      }
                      result.add(d2);
                      if (srcBB
                          .getNode()
                          .toString()
                          .contains("sendMessage(Ljava/lang/String;)V >")) {
    ...
    public IFlowFunction getReturnFlowFunction(
        ...
        BasicBlockInContext<IExplodedBasicBlock> src,
        BasicBlockInContext<IExplodedBasicBlock> dest) {
      final SSAAbstractInvokeInstruction callInst =
          (SSAAbstractInvokeInstruction) call.getDelegate().getInstruction();
        ...
        public IntSet getTargets(int d1) {
          ...
          if (node.equals(srcBlock.getNode())) {
            ...
            if (callInst.hasDef()) {
              ...
              Iterator<BasicBlockInContext<IExplodedBasicBlock>> iPredBB =
                  supergraph.getPredNodes(srcBlock);
      ...
      for (BasicBlockInContext<IExplodedBasicBlock> bb : supergraph) {
        ...
        if (instruction instanceof SSAAbstractInvokeInstruction) {
          ...
          if (!invokeInstr.isStatic()
              && invokeInstr.hasDef()
              && isSourceTarget(cgNode, invokeInstr)) {
            ...
            for (CGNode node : cg.getEntrypointNodes()) {
              for (BasicBlockInContext<IExplodedBasicBlock> bBlock :
                  supergraph.getEntriesForProcedure(node)) {
                Iterator<BasicBlockInContext<IExplodedBasicBlock>> iBlock =
                    supergraph.getSuccNodes(bBlock);
