...
public class UriCodecDecodeOpSetModel implements IOperationModel {
  ...
  public static UriCodecDecodeOpSetModel getInstance() {
    if (instance == null) instance = new UriCodecDecodeOpSetModel();
    return instance;
  }
  ...
  public IValue apply(IValue... args) {
    ...
    if (args.length != 4) throw new InternalError("UriCodecDecode method must have four in-edges.");
    ...
    if (absStr instanceof TopValue
        || absCs instanceof TopValue
        || absCv instanceof TopValue
        || absTof instanceof TopValue) return StringTopValue.getInstance();
    else if (absStr instanceof BotValue
        || absCs instanceof BotValue
        || absCv instanceof BotValue
        || absTof instanceof BotValue) return StringBotValue.getInstance();
    else if (absStr instanceof StringSetValue
        || absCs instanceof StringSetValue
        || absCv instanceof BooleanValue
        || absTof instanceof BooleanValue) {
      ...
      Assertions.UNREACHABLE(
          "incorrect args(arg1: "
              + absStr.getClass().getName()
              + ", arg2: "
              + absCv.getClass().getName()
              + ", arg3: "
              + absCs.getClass().getName()
              + ", arg4: "
              + absTof.getClass().getName()
              + ")");
    else return BotValue.getInstance();
    return null;
  }

  private String decode(String s, boolean convertPlus, Charset charset, boolean throwOnFailure) {
