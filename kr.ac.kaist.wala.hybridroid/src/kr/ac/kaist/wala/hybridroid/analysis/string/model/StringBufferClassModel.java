...
public class StringBufferClassModel extends AbstractClassModel {
  ...
  public static StringBufferClassModel getInstance() {
    if (instance == null) instance = new StringBufferClassModel();
    return instance;
  }
  ...
  class Append implements IMethodModel<Set<IBox>> {
    ...
    public Set<IBox> draw(
      ...
      switch (invokeInst.getNumberOfUses()) {
        ...
        default:
          StringModel.setWarning(
              "Unknown StringBuffer append: #arg is " + invokeInst.getNumberOfUses(), true);
      }
      return Collections.emptySet();
    }

    private Set<IBox> arg2(
        ConstraintGraph graph, IBox def, CGNode caller, SSAInvokeInstruction invokeInst) {
