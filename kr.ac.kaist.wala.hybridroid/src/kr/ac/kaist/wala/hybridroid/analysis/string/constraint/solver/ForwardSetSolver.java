...
public class ForwardSetSolver implements IStringConstraintSolver<IConstraintNode, IValue> {
  ...
  private boolean walk(ConstraintGraph graph, IConstraintNode n) {
    ...
    if (n instanceof IBox) {
      if (n instanceof ConstBox) {
        ...
        switch (cb.getType()) {
          ...
          case DOUBLE:
            ...
            } else nVal = doubleDomain.getOperator().alpha(cb.getValue());
            break;
          case UNKNOWN:
        ...
        if (preds.size() != 1) {
          nVal = StringBotValue.getInstance();
          // throw new InternalError("Incorrected Graph: Besides OperatorNode, all nodes must have
          // only one predecessor: [" + nindex + "] " + n + ", PREDS: " + preds.size());
        } else {
        ...
        if (preds.size() != 1) {
          String s = "";
          for (IConstraintNode nn : preds) s += " " + nn;
          throw new InternalError(
              "Incorrected Graph: Besides OperatorNode, all nodes must have only one predecessor: ["
                  + nindex
                  + "] "
                  + n
                  + ", PREDS: "
        ...
        throw new InternalError(
            "UnknownBox: IBox node must be ConstBox, ParamBox or VarBox: "
                + n.getClass().getName());
      ...
      try {
        ...
        if (DO_NOT_STOP) nVal = StringTopValue.getInstance();
  ...
  private void updateWorkList(
      Queue<IConstraintNode> worklist, ConstraintGraph graph, IConstraintNode n) {
    ...
    for (IConstraintNode to : graph.getSuccessors(n)) {
      if (!worklist.contains(to)) worklist.add(to);
    }
  }
  ...
  public Map<IConstraintNode, IValue> solve(ConstraintGraph graph) {
    ...
    while (!worklist.isEmpty()) {
      ...
      if (monitor != null) monitor.monitor(worklist, n, v, strMap.get(n), updateNeed);
    }
