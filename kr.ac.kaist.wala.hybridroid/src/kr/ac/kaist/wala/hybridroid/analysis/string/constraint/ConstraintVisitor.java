...
public final class ConstraintVisitor implements IBoxVisitor<Set<IBox>> {
  ...
  public ConstraintVisitor(
      CallGraph cg, FieldDefAnalysis fda, ConstraintGraph graph, IConstraintMonitor monitor) {
    this.cg = cg;
    this.fda = fda;
    this.graph = graph;
  ...
  private List<SSAReturnInstruction> getReturnInstructions(CGNode node) {
    if (returnInstCache.containsKey(node)) return returnInstCache.get(node);

    List<SSAReturnInstruction> returnList = new ArrayList<SSAReturnInstruction>();
    IR ir = node.getIR();

    if (ir == null) return returnList;

    SSAInstruction[] insts = ir.getInstructions();
    for (SSAInstruction inst : insts) {
      if (inst == null) continue;
      if (inst instanceof SSAReturnInstruction) returnList.add((SSAReturnInstruction) inst);
    }
  ...
  private List<SSAInvokeInstruction> getCallInstructions(CGNode caller, CGNode target) {
    ...
    if (callInstCache.containsKey(nodePair)) return callInstCache.get(nodePair);

    List<SSAInvokeInstruction> callList = new ArrayList<SSAInvokeInstruction>();
    IR ir = caller.getIR();

    if (ir == null) return callList;
  ...
  public Set<IBox> visit(VarBox b) {
    ...
    if (isArg(node, var)) {
      ...
      if (graph.addEdge(new AssignOpNode(), b, box)) res.add(box);
      ...
      if (defInst instanceof SSAInvokeInstruction) {
        ...
        if (cg.getPossibleTargets(node, invokeInst.getCallSite()).size() != 0) {
            ...
            if (m != null) {
              ...
              for (SSAReturnInstruction retInst : getReturnInstructions(target)) {
                if (retInst.getNumberOfUses() > 0) {
                  ...
                  if (graph.addEdge(new AssignOpNode(), b, retBox)) res.add(retBox);
                }
              }
            }
          }
          ...
          while (klass != null) {
            ...
            if (m != null) {
              res.addAll(m.draw(graph, b, node, invokeInst));
              break;
            } else klass = klass.getSuperclass();
          }
          if (m == null) {
        ...
        IUnaryOpInstruction.IOperator op = uopInst.getOpcode();
        IBox opBox = null;
        ...
        if (!StringModel.isClassModeled(nClass)) {
          ...
          //					StringModel.setWarning("an object of not modeled class is created: " + defInst,
          // true);
        ...
        if (graph.addEdge(new JoinOpNode(), b, boxes)) {
          for (int i = 0; i < boxes.length; i++) res.add(boxes[i]);
        }
    ...
    if (monitor != null) monitor.monitor(iter, graph, b, res);
  ...
  private boolean isArg(CGNode node, int var) {
    ...
    if ((method.getNumberOfParameters()) >= var) return true;
  ...
  private Object getConstant(CGNode node, int var) {
    ...
    if (symTab.isConstant(var)) {
      if (symTab.isNullConstant(var)) return "null";
      else if (symTab.isFalse(var)) return false;
      else if (symTab.isTrue(var)) return true;
      else return symTab.getConstantValue(var);
    }
    return null;
  }

  private ConstType getConstType(Object v) {
    if (v instanceof String) return ConstType.STRING;
    else if (v instanceof Boolean) return ConstType.BOOL;
    else if (v instanceof Integer) return ConstType.INT;
    else if (v instanceof Double) return ConstType.DOUBLE;
    else if (v instanceof Float) return ConstType.DOUBLE;
    else if (v instanceof Long) return ConstType.LONG;
  ...
  private SSAInvokeInstruction getInitInst(CGNode node, int def) {
    ...
    while (iInst.hasNext()) {
      ...
      if (inst instanceof SSAInvokeInstruction) {
        ...
        if (mr.isInit() && invokeInst.getUse(0) == def) return invokeInst;
      }
    }
