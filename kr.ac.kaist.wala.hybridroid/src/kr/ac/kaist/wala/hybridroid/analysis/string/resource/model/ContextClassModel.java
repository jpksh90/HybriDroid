...
public class ContextClassModel extends AbstractClassModel {
  ...
  public static ContextClassModel getInstance() {
    if (instance == null) instance = new ContextClassModel();
    return instance;
  }
  ...
  private String getString(CGNode node, int var) {
    ...
    if (ra != null && symTab.isConstant(var)) {
      ...
      String value = null;
      if (region != null) value = ra.getRegionString(addr, region);
      else value = ra.getCommonString(addr);

      if (value != null) return value;
      else {
        if (DEBUG) System.err.println("[Warning] the undefined string resource for " + addr);
        return "RESOURCE";
      }
    }
    ...
    // if the variable does not have constant value, we do not calculate it; just return "RESOURCE"
    // string value.
    return "RESOURCE";
  }
  ...
  class GetString1 implements IMethodModel<Set<IBox>> {
    ...
    public Set<IBox> draw(
      ...
      if (graph.addEdge(new AssignOpNode(), def, useBox)) boxSet.add(useBox);
      return boxSet;
    }
