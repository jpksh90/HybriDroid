...
public class ReplaceOpSetModel implements IOperationModel {
  ...
  public static ReplaceOpSetModel getInstance() {
    if (instance == null) instance = new ReplaceOpSetModel();
    return instance;
  }
  ...
  public IValue apply(IValue... args) {
    ...
    if (args.length != 3) throw new InternalError("Replace operation must have three in-edge.");
    ...
    if (absStr instanceof BotValue || absTarget instanceof BotValue || absSubst instanceof BotValue)
    ...
    else if (absStr instanceof TopValue
        || absTarget instanceof TopValue
        || absSubst instanceof TopValue) return StringTopValue.getInstance();
    else if (absStr instanceof StringSetValue
        && absTarget instanceof StringSetValue
        && absSubst instanceof StringSetValue) {
      ...
      Set<Integer> ss =
          ((Set<Integer>) (((IntegerSetValue) absStr).getDomain().getOperator().gamma(absStr)));
      if (ss.size() == 1 && ss.contains(0)) return IntegerBotValue.getInstance();
        ...
        Assertions.UNREACHABLE(
            "Incorrect args(arg1: "
                ...
                + ", arg2: "
                + absTarget.getClass().getName()
                + ", arg3: "
                + absSubst.getClass().getName()
                + ")");
