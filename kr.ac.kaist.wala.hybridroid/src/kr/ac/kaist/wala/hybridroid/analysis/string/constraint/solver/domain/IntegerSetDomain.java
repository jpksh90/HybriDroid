...
public final class IntegerSetDomain implements IIntegerDomain {
  ...
  public static IntegerSetDomain getDomain() {
    if (domain == null) domain = new IntegerSetDomain();
    return domain;
  }
  ...
  public static IntegerDomainOp op() {
    if (OP_INSTANCE == null) OP_INSTANCE = new IntegerDomainOp();
    return OP_INSTANCE;
  }
  ...
  public static class IntegerDomainOp implements IDomainOp<Integer, Set<Integer>> {
    ...
    public IIntegerValue alpha(Set<Integer> cvs) {
      // TODO Auto-generated method stub
      if (cvs.size() > MAX_SET_SIZE) return IntegerTopValue.getInstance();
      else return new IntegerSetValue(cvs);
    }
    ...
    public Set<Integer> gamma(IValue v) {
      ...
      if (v instanceof IntegerBotValue) {
        throw new InternalError(
            "Gamma function of String domain operation is applied to StringBotValue.");
      } else if (v instanceof IntegerTopValue) {
        throw new InternalError(
            "Gamma function of String domain operation is applied to StringTopValue.");
      } else if (v instanceof BotValue) {
        throw new InternalError(
            "Gamma function of String domain operation is applied to BotValue.");
      } else if (v instanceof TopValue) {
        throw new InternalError(
            "Gamma function of String domain operation is applied to TopValue.");
      } else if (v instanceof IntegerSetValue) {
        IntegerSetValue ssv = (IntegerSetValue) v;
        return ssv.values;
      } else {
        throw new InternalError(
            "String domain operation is not compatible to " + v.getClass().getName());
      }
    }
  }
  ...
  public static class IntegerSetValue implements IIntegerValue {
    ...
    public IValue weakUpdate(IValue v) {
      // TODO Auto-generated method stub
      if (v instanceof BotValue) return this;
      else if (v instanceof IIntegerValue) {
        if (v instanceof IntegerBotValue) return this;
        else if (v instanceof IntegerTopValue) return v;
          ...
          if (MAX_SET_SIZE < newValues.size()) return IntegerTopValue.getInstance();
