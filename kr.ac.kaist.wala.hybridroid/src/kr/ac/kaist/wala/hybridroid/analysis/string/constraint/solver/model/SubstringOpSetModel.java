...
public class SubstringOpSetModel implements IOperationModel {
  ...
  public static SubstringOpSetModel getInstance() {
    if (instance == null) instance = new SubstringOpSetModel();
    return instance;
  }
      ...
      if (ss.size() == 1 && ss.contains(0)) return IntegerBotValue.getInstance();
        ...
        Assertions.UNREACHABLE(
            "Incorrect args(arg1: "
                ...
                + ", arg2: "
                + begin.getClass().getName()
                + ")");
      else return BotValue.getInstance();
    }

    if (args.length == 2) {
      if (str instanceof TopValue || begin instanceof TopValue) return StringTopValue.getInstance();
        ...
        for (String s : ss) {
          for (Integer i : is) {
            if (s.length() >= i && i > -1) res.add(s.substring(i));
          }
        }
        return StringSetDomain.op().alpha(res);
      } else
        Assertions.UNREACHABLE(
            "Incorrect args types(arg1: "
                + str.getClass().getName()
                + ", arg2: "
                + begin.getClass().getName());
      ...
      if (str instanceof TopValue || begin instanceof TopValue || end instanceof TopValue)
        ...
        return StringBotValue.getInstance();
          ...
          && begin instanceof IntegerSetValue
          && end instanceof IntegerSetValue) {
        Set<String> ss = (Set<String>) str.getDomain().getOperator().gamma(str);
        ...
        for (String s : ss) {
          for (Integer bi : bis) {
            for (Integer ei : eis) {
              if (s.length() >= ei && bi <= ei && bi > -1) res.add(s.substring(bi, ei));
            }
          }
        }
        return StringSetDomain.op().alpha(res);
