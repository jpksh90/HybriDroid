...
public class ToStringOpSetModel implements IOperationModel {
  ...
  public static ToStringOpSetModel getInstance() {
    if (instance == null) instance = new ToStringOpSetModel();
    return instance;
  }
  ...
  public IValue apply(IValue... args) {
    ...
    if (args.length != 1) throw new InternalError("ToString operation must have only one in-edge.");

    IValue arg = args[0];

    if (arg instanceof TopValue) return StringTopValue.getInstance();
