...
/**
 * Specialized pointer analysis constraint generation for Hybrid Android applications. Now, this
 * support only 'addJavascriptInterface' Android Java method among several communication methods
 * between Android Java and JavaScript.
 *
 * <p>We need to support some callback communication methods, but not now.
 */
public class AndroidHybridCallGraphBuilder extends JavaJavaScriptHybridCallGraphBuilder {
  ...
  private final Map<Atom, GlobalObjectKey> jsGlobalMap;

  public AndroidHybridCallGraphBuilder(
      IMethod fakeRootClass,
      AnalysisOptions options,
      AnalysisCache cache,
      HybridAPIMisusesChecker typeChecker,
      AndroidStringAnalysis asa,
    ...
    if ((options.getAnalysisScope() instanceof AndroidHybridAnalysisScope) == false)
      Assertions.UNREACHABLE(
          "AndroidHybridCallGraphBuilder must receive AndroidHybridAnalysisScope as a scope. current: "
              + options.getAnalysisScope().getClass().getName());
    ...
    TypeReference jsinterAnnTR =
        TypeReference.find(ClassLoaderReference.Primordial, "Landroid/webkit/JavascriptInterface");
  ...
  private FilteredPointerKey getFilteredPointerKeyForInterfParams(
      CGNode caller,
    ...
    if (AndroidJavaJavaScriptTypeMap.isJava2JsTypeCompatible(filter.getReference())) {
      return getFilteredPointerKeyForLocal(
          node,
          valueNumber,
          JSCompatibleClassFilter.make(caller, inst, node, valueNumber, typeChecker, filter));
    } else {
  ...
  public PointerKey getTargetPointerKey(
      CGNode caller, SSAAbstractInvokeInstruction inst, CGNode target, int index) {
    int vn;
  ...
  private void processHybridCallingConstraints(
      CGNode caller,
    ...
    for (int i = 0; i < Math.min(paramCount, argCount); i++) {
      ...
      if (constParams != null && constParams[i + 1] != null) {
        ...
        this.getSystem()
            .newConstraint(
                F, (F instanceof FilteredPointerKey) ? this.filterOperator : assignOperator, A);
      }
    }
    ...
    if (instruction.getDef(0) != -1) {
      ...
      if (typeChecker != null) typeChecker.returnTypeCheck(caller, instruction, target.getMethod());
      ...
      PointerKey RA =
          this.getFilteredPointerKeyForLocal(
              caller,
              instruction.getDef(0),
              JavaCompatibleClassFilter.make(caller, instruction, target, typeChecker));
  ...
  protected void processCallingConstraints(
    ...
    if (JavaScriptLoader.JS.equals(
      ...
      if (constParams != null
        ...
        processHybridCallingConstraints(caller, instruction, target, constParams, uniqueCatchKey);
      } else
  ...
  /**
   * checking the method has '@JavascriptInterface' annotation. the annotation shows that the method
   * can be called at a JavaScript call site.
   *
   * @param m target method
   * @return true if the method has '@JavascriptInterface' annotation, false otherwise.
   */
  private boolean hasJavascriptInterfaceAnnotation(IMethod m) {
    if (annVersion && m.getAnnotations() != null) {
      for (Annotation ann : m.getAnnotations()) {
        ...
        if (jsinterAnnClass.equals(cha.lookupClass(annTr))) return true;
      }
      return false;
    } else return true;
  }
  ...
  protected PropagationSystem makeSystem(AnalysisOptions options) {
    return new PropagationSystem(callGraph, pointerKeyFactory, instanceKeyFactory) {
      ...
      public PointerAnalysis<InstanceKey> makePointerAnalysis(PropagationCallGraphBuilder builder) {
        ...
        return new AndroidHybridPointerAnalysisImple(
            AndroidHybridCallGraphBuilder.this,
            cg,
            pointsToMap,
            instanceKeys,
            pointerKeyFactory,
  ...
  protected TargetLanguageSelector<AstImplicitPointsToSetVisitor, LocalPointerKey>
    ...
    return new TargetLanguageSelector<AstImplicitPointsToSetVisitor, LocalPointerKey>() {
      ...
      public AstImplicitPointsToSetVisitor get(Atom language, LocalPointerKey construct) {
        if (JavaScriptTypes.jsName.equals(language)) {
          return new AndroidHybridJSImplicitPointsToSetVisitor(
  ...
  public class HybridJavaConstraintVisitor extends ResourceVisitor {
    ...
    public void visitInvoke(SSAInvokeInstruction instruction) {
      ...
      if ((receiver != null && (wvClass.equals(receiver) || cha.isSubclassOf(receiver, wvClass)))
        ...
        if (DEBUG) System.out.println("Invoked addJavascriptInterface.");
            ...
            if (symTab.isConstant(nameUse)) {
              ...
              for (InstanceKey objIK : objIKSet) {
                ...
                for (IField field : objIK.getConcreteType().getAllFields()) {
                  PointerKey javaFieldKey =
                      pa.getHeapModel().getPointerKeyForInstanceField(objIK, field);
                  PointerKey jsFieldKey =
                      pa.getHeapModel().getPointerKeyForInstanceField(objKeys[objIndex], field);
              ...
              for (int i = 0; i < objKeys.length; i++) {
                AbstractFieldPointerKey typePK =
                    ReflectedFieldPointerKey.mapped(new ConcreteTypeKey(jsStringClass), objKeys[i]);
                ...
                for (IMethod method : methods) {
                  if (hasJavascriptInterfaceAnnotation(method)) {
                    ...
                    if (DEBUG)
                      System.out.println(
                          "\t\tBridgeMethod: "
                              + ((InstanceFieldKey) constantPK).getField().getFieldTypeReference()
                              + " -> "
                              + mname);
              ...
              for (HotspotDescriptor desc : descSet) {
                if (desc.getReceiverAlias().contains(Pointing.make(node, instruction.getUse(0)))) {
                  ...
                  // if at least one of string analysis is fail, all html files can be loaded for
                  // sound approaches.
                  ...
                  if (htmls.size() == 1
                      && htmls.iterator().next().equals("about:blank")) { // no-op.
                    continue;
                    ...
                    for (String html : htmls) {
                      ...
                      //												if(DEBUG)
                      //													System.out.println("Load online page: " + html +", we do not
                      // deal with it.");
                      ...
                      if (js != null) {
                        ...
                        if (DEBUG) {
                          String objs = "";
                          for (InstanceKey ik : objKeys) objs += ik + ", ";
                    ...
                    if (isOnlineConnection
                        && globalObjs.isEmpty()) { // no-op; for online connection.
                      continue;
                    }
                  }
                } else {
              ...
              FieldReference field =
                  FieldReference.findOrCreate(
                      JavaScriptTypes.jsLoader, "LRoot", "global " + name, "LRoot");
              String nonGlobalFieldName = field.getName().toString().substring(7);
              field =
                  FieldReference.findOrCreate(
                      JavaScriptTypes.Root,
    ...
    /**
     * making a mock-up instance key for the method which can be called from a JavaScript call site.
     *
     * @param method the target method used for making the mock-up instance.
     * @return
  ...
  protected class AndroidHybridPointerAnalysisImple extends CrossLanguagePointerAnalysisImpl {
    protected AndroidHybridPointerAnalysisImple(
  ...
  class AndroidHybridJSConstraintVisitor extends JSConstraintVisitor {
    public AndroidHybridJSConstraintVisitor(
              ...
              class FieldValueDispatch extends UnaryOperator<PointsToSetVariable> {
                ...
                public byte evaluate(PointsToSetVariable lhs, PointsToSetVariable ptrs) {
                  if (receiverType.getConcreteType() instanceof InterfaceClass) {
                    if (DEBUG) System.out.println("==> " + fieldKey + " (" + ptrs.getValue() + ")");
    ...
    public void visitAstGlobalRead(AstGlobalRead instruction) {
      ...
      if (hasNoInterestingUses(lval)) {
        ...
        if ((method instanceof AstMethod) == false)
          Assertions.UNREACHABLE(
              "Global read must be invoked in AstMethod: " + method.getClass().getName());
  ...
  public class AndroidHybridJSImplicitPointsToSetVisitor extends JSImplicitPointsToSetVisitor {
    ...
    public AndroidHybridJSImplicitPointsToSetVisitor(
        AstPointerAnalysisImpl analysis, LocalPointerKey lpk) {
      super(analysis, lpk);
    }
