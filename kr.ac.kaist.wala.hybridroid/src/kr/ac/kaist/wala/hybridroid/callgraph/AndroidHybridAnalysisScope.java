...
/**
 * AnalysisScope for Android Hybrid application. This class supports DROIDEL Dex scope or original
 * Android platform scope as well as JavaScript scope.
 *
 * @author Sungho Lee
 */
public class AndroidHybridAnalysisScope extends AnalysisScope {
  ...
  /**
   * Make AnalysisScope for Android hybrid application. If you want to use DROIDEL as front-end, use
   * setUpDroidelAnalysisScope method instead of this.
   *
   * @param classpath the target apk file uri.
   * @param htmls JavaScript files contained in the scope.
   ...
   * @throws IOException
   */
  public static AndroidHybridAnalysisScope setUpAndroidHybridAnalysisScope(
      String dir, URI classpath, Set<URL> htmls, String exclusions, URI... androidLib)
      throws IOException {
    ...
    if (!htmls.isEmpty()) scope = setUpJsAnalysisScope(dir, scope, htmls);
  ...
  private static AndroidHybridAnalysisScope setUpJsAnalysisScope(
    ...
    if (SystemUtils.IS_OS_WINDOWS) {
      ...
      // scope.addToScope(scope.getJavaScriptLoader(),
      // JSCallGraphBuilderUtil.getPrologueFile("preamble.js"));
    }
    for (URL url : htmls) {
      try {
        ...
        if (url.toString().startsWith("http")) {
          ...
          if (DEBUG) System.err.println("make dummy: " + dummypath);
          addScopeMap(
              Atom.findOrCreateAsciiAtom(url.toString()),
              Atom.findOrCreateAsciiAtom(
                  dummypath.substring(dummypath.lastIndexOf(File.separator) + 1)));
  ...
  private static void addScopeMap(Atom f, Atom s) {
    ...
    if (DEBUG) System.out.println("[scope] " + f + " => " + s);
  }
