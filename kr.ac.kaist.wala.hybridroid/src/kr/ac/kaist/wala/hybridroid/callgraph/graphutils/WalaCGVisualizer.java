...
public class WalaCGVisualizer {
  ...
  private boolean isAndroidLibrary(CGNode n) {
    ...
    if (n.getMethod().getDeclaringClass() != null
        && n.getMethod().getDeclaringClass().getReference().getName().getPackage() != null)
      if (n.getMethod()
              .getDeclaringClass()
              .getReference()
              .getName()
          ...
          || n.getMethod()
              ...
              .getPackage()
              .toString()
              .startsWith("com/google/")) return true;
    return false;
  }

  private boolean isFiltered(CGNode n) {
    if (n.toString().contains("ctor")
        || n.toString().contains("preamble.js")
        || n.toString().contains("prologue.js")
        || n.getMethod()
            .getDeclaringClass()
            .getClassLoader()
            .getReference()
        ...
        || n.getMethod()
            ...
            .getClassLoader()
            .getReference()
            .equals(ClassLoaderReference.Extension)) return true;
    return false;
  }
    ...
    for (CGNode node : cg) {
      if (!isFiltered(node)) {
        ...
        while (iPredNodes.hasNext()) {
          ...
          } else vis.fromAtoB(getLabel(cg, predNode), getLabel(cg, node));
        }
      }
    }
    vis.printGraph(out);
  ...
  private String getLabel(CallGraph cg, CGNode node) {
    ...
    if (labelMap.containsKey(node)) return labelMap.get(node);
  ...
  public File transforms(File f, String tname) throws IOException {
    ...
    ProcessBuilder pb =
        new ProcessBuilder("/usr/local/bin/dot", "-Tsvg", "-o", tname, "-v", f.getCanonicalPath());
    pb.directory(new File(dir));
    Map<String, String> envs = pb.environment();
    //		envs.put("PATH", envs.get("PATH") + ":" + "/usr/local/bin/");
    System.err.println(
        "#translating " + f.getCanonicalPath() + " to " + dir + File.separator + tname);
    try {
      Process tr = pb.start();
