...
public class ResourceCallGraphBuilder extends ZeroXCFABuilder {
  ...
  private ResourceCallGraphBuilder(
      IClassHierarchy cha,
  ...
  public static ResourceCallGraphBuilder make(
      IClassHierarchy cha,
      AnalysisOptions options,
      AnalysisCache cache,
      ContextSelector appContextSelector,
    ...
    return new ResourceCallGraphBuilder(
        cha, options, cache, appContextSelector, appContextInterpreter, instancePolicy, ara);
  }
  ...
  public static class ResourceVisitor extends ConstraintVisitor {
    ...
    public ResourceVisitor(
        SSAPropagationCallGraphBuilder builder, CGNode node, AndroidResourceAnalysis ara) {
      super(builder, node);
      // TODO Auto-generated constructor stub
      cha = builder.cha;
      this.ara = ara;
      ...
      TypeReference dialogTR =
          TypeReference.find(ClassLoaderReference.Primordial, "Landroid/app/Dialog");
      TypeReference viewTR =
          TypeReference.find(ClassLoaderReference.Primordial, "Landroid/view/View");
      TypeReference windowTR =
          TypeReference.find(ClassLoaderReference.Primordial, "Landroid/view/Window");
      fakeRootTR =
          TypeReference.findOrCreate(
              ClassLoaderReference.Primordial, "Lcom/ibm/wala/FakeRootClass");
    ...
    public void visitInvoke(SSAInvokeInstruction instruction) {
      ...
      if (target.getSelector().equals(HybriDroidTypes.SETWEBVIEWCLIENT_SELECTOR)
          && (mainClass.getReference().equals(HybriDroidTypes.WEBVIEW_PRI_CLASS)
              || mainClass.getReference().equals(HybriDroidTypes.WEBVIEW_APP_CLASS))) {
        ...
        if (objType != null) {
          ...
          if (!system.isImplicit(webviewPK)) {
            for (IMethod m : objClass.getAllMethods()) {
              if (m.getDeclaringClass().equals(objClass)
                  && m.getName().toString().startsWith("on")) {
                try {
                  CGNode callbackNode =
                      builder.getCallGraph().findOrCreateNode(m, Everywhere.EVERYWHERE);
                  if (!system.isImplicit(builder.getPointerKeyForLocal(callbackNode, 2))) {
                    boolean n =
                        system.newConstraint(
                            builder.getPointerKeyForLocal(callbackNode, 2),
                            assignOperator,
                            webviewPK);
                  }
      ...
      } else if (fvbiSelector.equals(target.getSelector())
          && mainClass != null
          && (cha.isSubclassOf(mainClass, activityClass)
              || mainClass.equals(viewClass)
              || cha.isSubclassOf(mainClass, viewClass)
              || mainClass.equals(dialogClass)
              || cha.isSubclassOf(mainClass, dialogClass)
              || mainClass.equals(windowClass)
              || cha.isSubclassOf(mainClass, windowClass))) {
        SymbolTable symTab = node.getIR().getSymbolTable();
        int paramVar = instruction.getUse(1);
        int defVar = instruction.getDef();
        SSACheckCastInstruction ccInst =
            getCastingInstructionOf(node.getIR().getInstructions(), instruction.iIndex(), defVar);

        if (symTab.isIntegerConstant(paramVar)) {
          int v = symTab.getIntValue(paramVar);

          if (ccInst != null) {
            // at this point, instancekey may be exploit when the result is not converted to
            // concrete type.
            for (TypeReference tr : ccInst.getDeclaredResultTypes()) {
              ...
              ResourceInstanceKey rik =
                  new ResourceInstanceKey(node, klass, instruction.iIndex(), v);
          ...
          if (resId < 0) {
            if (DEBUG)
              System.err.println(
                  "Warning: resource descriptor is not int constant."
                      + instruction
                      + "("
                      + symTab.isConstant(paramVar)
                      + ") in "
                      + node);
          } else {
            int v = resId;
            if (ccInst != null) {
              for (TypeReference tr : ccInst.getDeclaredResultTypes()) {
                IClass klass = cha.lookupClass(tr);
                if (klass == null) Assertions.UNREACHABLE(tr + " does not exist.");
                ResourceInstanceKey rik =
                    new ResourceInstanceKey(node, cha.lookupClass(tr), instruction.iIndex(), v);
                system.newConstraint(builder.getPointerKeyForLocal(node, defVar), rik);
              }
              return;
            } else {
              ResourceInstanceKey rik =
                  new ResourceInstanceKey(
                      ...
                      cha.lookupClass(
                          TypeReference.find(
                              ClassLoaderReference.Primordial, "Landroid/view/View")),
                      instruction.iIndex(),
                      v);
              system.newConstraint(builder.getPointerKeyForLocal(node, defVar), rik);
              return;
      ...
      } else if (setidSelector.equals(target.getSelector())
          && mainClass != null
          && (viewClass.equals(mainClass) || cha.isSubclassOf(mainClass, viewClass))) {
        SymbolTable symTab = node.getIR().getSymbolTable();
        int receiverVar = instruction.getUse(0);
        int paramVar = instruction.getUse(1);
        int v = -1;

        if (symTab.isIntegerConstant(paramVar)) v = symTab.getIntValue(paramVar);
        else v = getStaticDefinedIntValue(node, paramVar);

        if (v < 0) {
          if (DEBUG)
            System.err.println(
                "Warning: resource descriptor is not an int constant."
                    + instruction
                    + "("
                    + symTab.isConstant(paramVar)
                    + ") in "
          ...
          if (symTab.isParameter(receiverVar)) {
            ...
            IClass klass = cha.lookupClass(tr);
            if (klass == null) Assertions.UNREACHABLE(tr + " does not exist in this application.");
            ...
            if (wtr.has()) {
              ...
              if (klass == null) Assertions.UNREACHABLE(wtr.getObject() + " does not exist.");

              ResourceInstanceKey rik =
                  new ResourceInstanceKey(node, klass, instruction.iIndex(), v);

              try {
                if (!system.isImplicit(pk)) system.newConstraint(pk, rik);
              } catch (
                  UnimplementedError
                      e) { // if the pointer key is implicit, then just return; work normally.
                return;
              }
            }
          }
    ...
    private SSACheckCastInstruction getCastingInstructionOf(
        SSAInstruction[] insts, int startIndex, int var) {
      ...
      do {
        ...
        if (nextInst != null
            && nextInst instanceof SSACheckCastInstruction
            && nextInst.getUse(0) == var) return (SSACheckCastInstruction) nextInst;
    ...
    private int getStaticDefinedIntValue(CGNode n, int var) {
      ...
      if (defInst == null || !(defInst instanceof SSAGetInstruction)) return -1;
