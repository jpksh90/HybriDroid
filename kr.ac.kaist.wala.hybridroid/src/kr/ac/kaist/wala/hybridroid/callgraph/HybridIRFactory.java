...
public class HybridIRFactory extends DefaultIRFactory {
  ...
  public IR makeIR(IMethod method, Context c, SSAOptions options) throws IllegalArgumentException {
    ...
    if (method instanceof DexIMethod) return dexIRFactory.makeIR(method, c, options);
    else return jsIRFactory.makeIR(method, c, options);
  }

  @Override
  public boolean contextIsIrrelevant(IMethod method) {
    // TODO Auto-generated method stub
    if (method instanceof DexIMethod) return dexIRFactory.contextIsIrrelevant(method);
    else return jsIRFactory.contextIsIrrelevant(method);
  }
}
