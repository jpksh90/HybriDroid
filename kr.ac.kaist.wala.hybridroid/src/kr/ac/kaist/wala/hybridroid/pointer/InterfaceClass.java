...
public class InterfaceClass extends AstClass implements IClass {
  ...
  protected InterfaceClass(Position pos, IClass targetClass) {
    super(
        pos,
        targetClass.getReference().getName(),
  ...
  public static InterfaceClass wrapping(final IClass javaClass, final IClass superClass) {
    if (jsrootClass == null) jsrootClass = superClass;

    if (classes.keySet().contains(javaClass)) return classes.get(javaClass);
  ...
  public void addMethodAsField(IMethod method) {
    FieldReference fr =
        FieldReference.findOrCreate(
            javaClass.getClassLoader().getReference(),
            javaClass.getReference().getName().toString(),
            method.getName().toString(),
            method.getReference().getName().toString());
    IField f =
        new FieldImpl(
            javaClass, fr, getAccessFlagForMethod(method), (Collection) Collections.emptySet());
  ...
  public IField getField(Atom name) {
    ...
    if (f == null) {
      if (mFields.containsKey(name)) return mFields.get(name);
