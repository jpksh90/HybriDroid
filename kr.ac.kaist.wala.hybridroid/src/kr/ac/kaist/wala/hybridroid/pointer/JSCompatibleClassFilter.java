...
/**
 * JavaScript to Android Java type compatible class filter. This filter can be used for a parameter
 * of bridge methods. The input JavaScript argument is converted to corresponding Android Java
...
public class JSCompatibleClassFilter extends SingleClassFilter {
  ...
  private JSCompatibleClassFilter(
      CGNode caller,
  ...
  public static JSCompatibleClassFilter make(
      CGNode caller,
      SSAAbstractInvokeInstruction inst,
      CGNode target,
      int argNum,
      HybridAPIMisusesChecker typeChecker,
      IClass concreteClass) {
  ...
  private PointsToSetVariable convert2JSType(
      final PropagationSystem system, PointsToSetVariable R) {
                ...
                public void act(int x) {
                  ...
                  if (AndroidJavaJavaScriptTypeMap.isJs2JavaTypeCompatible(jsType, javaType)) {
                    ...
                    if (ik instanceof ConcreteTypeKey) {
                      ...
                      convertedKey =
                          new ConstantKey(((ConstantKey) ik).getValue(), getConcreteType());
                    } else
                  ...
                  typeChecker.argTypeCheck(
                        caller,
                        inst,
                        argNum,
                        target.getMethod(),
                        getConcreteType().getReference(),
                        ik.getConcreteType().getReference());
                }
              });
    }
