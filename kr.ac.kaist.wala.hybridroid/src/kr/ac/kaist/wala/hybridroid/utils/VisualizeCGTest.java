...
public class VisualizeCGTest {
  ...
  static {
    if (System.getProperty("os.name").matches(".*Mac.*")) DOT_PATH = "/usr/local/bin/dot";
  ...
  private static void assignNodeName(CallGraph cg, FileOutputStream outStream) throws IOException {
    for (CGNode node : cg) {
      if (isVisualizable(node)) {
        String irs = "";
        if (node.getIR() != null) irs = node.getIR().toString();
        int num = 0;
        String before = irs;
        while (irs.indexOf("\n\n") > 0) {
          irs = irs.replace("\n\n", "\n");
        ...
        while (iSuccNode.hasNext()) {
          ...
          if (iSuccNode.hasNext()) irs += "\\l";
        }
        // irs = irs.replace("@", "(at)");
        if (isContentPrintable(node))
          outStream.write(
              (cg.getNumber(node) + " [label=\"" + node + "\n" + irs + "\" shape=box]\n")
                  .getBytes());
        else
          outStream.write(
              (cg.getNumber(node) + " [label=\"" + node + "\" shape=box]\n").getBytes());
      }
    }
  }
        ...
        if (isVisualizable(node))
          for (Iterator<CGNode> succI = cg.getSuccNodes(node); succI.hasNext(); ) {
            ...
            if (isVisualizable(succNode)) {
              outStream.write(
                  (cg.getNumber(node) + " -> " + cg.getNumber(succNode) + "\n").getBytes());
            }
          }
      }
    ...
    if (convert) {
      ...
      ProcessBuilder pb =
          new ProcessBuilder(
              ...
              "-Tsvg",
              "-o",
              dirpath + File.separator + _outputFileName + ".svg",
              "-v",
              filepath);
      pb.directory(folder);
