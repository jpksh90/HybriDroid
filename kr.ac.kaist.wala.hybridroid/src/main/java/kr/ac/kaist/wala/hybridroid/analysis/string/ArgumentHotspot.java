...
public class ArgumentHotspot implements Hotspot {
  ...
  public ArgumentHotspot(
      ClassLoaderReference cRef, String cDescriptor, String mDescriptor, int argIndex) {
    this.argIndex = argIndex;

    this.cDescriptor =
        TypeReference.findOrCreateClass(
            cRef,
            cDescriptor.substring(0, cDescriptor.lastIndexOf("/")),
            cDescriptor.substring(cDescriptor.lastIndexOf("/") + 1));
