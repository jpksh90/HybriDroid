...
public class LongSetDomain implements ILongDomain {
  ...
  public static LongSetDomain getDomain() {
    if (domain == null) domain = new LongSetDomain();
    return domain;
  }
  ...
  public static LongDomainOp op() {
    if (OP_INSTANCE == null) OP_INSTANCE = new LongDomainOp();
    return OP_INSTANCE;
  }
  ...
  static class LongDomainOp implements IDomainOp<Long, Set<Long>> {
    ...
    public ILongValue alpha(Set<Long> cvs) {
      // TODO Auto-generated method stub
      if (cvs.size() > MAX_SET_SIZE) return LongTopValue.getInstance();
      else return new LongSetValue(cvs);
    }
    ...
    public Set<Long> gamma(IValue v) {
      ...
      if (v instanceof LongBotValue) {
        throw new InternalError(
            "Gamma function of String domain operation is applied to LongBotValue.");
      } else if (v instanceof LongTopValue) {
        throw new InternalError(
            "Gamma function of String domain operation is applied to LongTopValue.");
        ...
        throw new InternalError(
            "Long domain operation is not compatible to " + v.getClass().getName());
      }
    }
  }
  ...
  public static class LongSetValue implements ILongValue {
    ...
    public IValue weakUpdate(IValue v) {
      ...
      if (v instanceof BotValue) return this;
        ...
        if (v instanceof LongBotValue) return this;
        else if (v instanceof LongTopValue) return v;
        else {
          LongSetValue ssv = (LongSetValue) v;
          Set<Long> newValues = new HashSet<Long>();
          newValues.addAll(values);
          newValues.addAll(ssv.values);
          if (MAX_SET_SIZE < newValues.size()) return LongTopValue.getInstance();
